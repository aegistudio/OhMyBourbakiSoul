...
theorem demorgan_univ_bigcup {A : MySet (MySet α)}:
  ...
  apply Iff.intro
  · intro hxC
    rw [complement_def] at hxC
    rw [bigcup_def] at hxC
    rw [bigcap_def]
    intro a ha
...
theorem bigcap_complement_exclusive_mem_if_bigcup_exclusive_mem
  ...
  · intro hxA
    ...
    right
    intro hxAc
    rw [bigcap_def] at hxAc
    have ha'Ac : aᶜ ∈ { sᶜ || s ∈ A } := by
      exists a
      exact And.intro (And.left ha) rfl
    have hxa := And.right ha
    have hxa' := hxAc aᶜ ha'Ac
    rw [complement_def] at hxa'
    contradiction
  · intro hxA'
    left
    ...
    have : ∃ s ∈ A, x ∈ s := by
      exists a
      exact And.intro (And.left ha) hxa
    contradiction

theorem demorgan_univ_bigcap_if_exclusive_mem
  {A : MySet (MySet α)} :
  (∀ (x : α), ∀ a ∈ A, (x ∈ a) ∨ (x ∉ a)) →
  ⋂ A = (⋃ { aᶜ || a ∈ A })ᶜ := by
  intro hem
  have hdc : ∀ a ∈ A, a = (aᶜ)ᶜ := by
    intro a haA
    apply double_complement_eq_self_if_exclusive_mem
    intro x
    exact hem x a haA
  have hAAcc : A = { (aᶜ)ᶜ || a ∈ A } := by
    rw [eq_iff]
    intro a
    apply Iff.intro
    · intro haA
      rw [transform_def]
      exists a
      exact And.intro haA (hdc a haA)
    · intro haTA
      rw [transform_def] at haTA
      rcases haTA with ⟨a', ha'⟩
      have h' := hdc a' (And.left ha')
      rw [<-And.right ha'] at h'
      rw [<-h']
      exact And.left ha'
  have h := demorgan_univ_bigcup (A := { aᶜ || a ∈ A })
  rw [transform_compose] at h
  symm at h
  rw [<-hAAcc] at h
  exact h
